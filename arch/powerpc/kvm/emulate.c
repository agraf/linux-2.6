...
struct kvmppc_opentry *kvmppc_list_spr_r;
struct kvmppc_opentry *kvmppc_list_spr_w;
...
static int kvmppc_emulate_sthx(struct kvm_vcpu *vcpu, int rs, int ra, int rb,
			       int rc)
{
	ulong val = kvmppc_get_gpr(vcpu, rs);
	return kvmppc_handle_store(vcpu->run, vcpu, val, 2, 1);
...
static int kvmppc_emulate_sthux(struct kvm_vcpu *vcpu, int rs, int ra, int rb,
			       int rc)
{
	int r;
	ulong val = kvmppc_get_gpr(vcpu, rs);
...
static int kvmppc_emulate_tlbsync(struct kvm_vcpu *vcpu, int rt, int ra, int rb,
				  int rc)
{
	return EMULATE_DONE;
}

static int kvmppc_emulate_stwbrx(struct kvm_vcpu *vcpu, int rs, int ra, int rb,
				 int rc)
{
	ulong val = kvmppc_get_gpr(vcpu, rs);
	return kvmppc_handle_store(vcpu->run, vcpu, val, 4, 0);
}

static int kvmppc_emulate_lhbrx(struct kvm_vcpu *vcpu, int rt, int ra, int rb,
				int rc)
{
	return kvmppc_handle_load(vcpu->run, vcpu, rt, 2, 0);
}

static int kvmppc_emulate_sthbrx(struct kvm_vcpu *vcpu, int rs, int ra, int rb,
				 int rc)
{
	ulong val = kvmppc_get_gpr(vcpu, rs);
	return kvmppc_handle_store(vcpu->run, vcpu, val, 2, 0);
}
...
static int kvmppc_emulate_mfmsr(struct kvm_vcpu *vcpu, int rt, int ra, int rb,
				 int rc)
{
...
static int kvmppc_spr_write_srr0(struct kvm_vcpu *vcpu, int sprn, ulong val)
{
	vcpu->arch.shared->srr0 = val;
	return EMULATE_DONE;
}

static int kvmppc_spr_read_srr1(struct kvm_vcpu *vcpu, int sprn, ulong *val)
{
	*val = vcpu->arch.shared->srr1;
	return EMULATE_DONE;
}

static int kvmppc_spr_write_srr1(struct kvm_vcpu *vcpu, int sprn, ulong val)
{
	vcpu->arch.shared->srr1 = val;
	return EMULATE_DONE;
}
...
static int kvmppc_spr_read_pir(struct kvm_vcpu *vcpu, int sprn, ulong *val)
{
	*val = vcpu->vcpu_id;
	return EMULATE_DONE;
}
...
static int kvmppc_spr_read_sprg0(struct kvm_vcpu *vcpu, int sprn, ulong *val)
{
	*val = vcpu->arch.shared->sprg0;
	return EMULATE_DONE;
}

static int kvmppc_spr_write_sprg0(struct kvm_vcpu *vcpu, int sprn, ulong val)
{
	vcpu->arch.shared->sprg0 = val;
	return EMULATE_DONE;
}

static int kvmppc_spr_read_sprg1(struct kvm_vcpu *vcpu, int sprn, ulong *val)
{
	*val = vcpu->arch.shared->sprg1;
	return EMULATE_DONE;
}

static int kvmppc_spr_write_sprg1(struct kvm_vcpu *vcpu, int sprn, ulong val)
{
	vcpu->arch.shared->sprg1 = val;
	return EMULATE_DONE;
}
...
static int kvmppc_spr_read_dec(struct kvm_vcpu *vcpu, int sprn, ulong *val)
{
	*val = kvmppc_get_dec(vcpu, get_tb());
	return EMULATE_DONE;
}

static int kvmppc_spr_write_dec(struct kvm_vcpu *vcpu, int sprn, ulong val)
{
	vcpu->arch.dec = val;
...
void __init kvmppc_emulate_register_spr(int sprn, int flags,
	...
	int (*write)(struct kvm_vcpu *vcpu, int sprn, ulong val))
{
	...
	sprn &= 0x3ff;
	kvmppc_list_spr_r[sprn] = entry_r;
	kvmppc_list_spr_w[sprn] = entry_w;
}
